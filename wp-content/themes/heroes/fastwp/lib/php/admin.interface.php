...
class fwp_admin_ui {
	...
	static function el_switch($id, $value = '0', $title='',  $description='', $wrap_class = '', $labels = ''){
		...
		$labelOn		= esc_html__('On','heroes');
		$labelOff		= esc_html__('Off','heroes');
		if($title == ''){
	...
	static function el_gallery($id, $value = '', $title='',  $description='', $wrap_class = ''){
		...
		$items[] 	= sprintf('<div class="fwp-no-results %s">%s</div>', ((is_array($value) && count($value) > 0)?'fwp-hidden':''), esc_html__('No items in gallery', 'heroes'));
		if(is_array($value)){
