<?php
add_action('init', 						array('FastWP_Actions', 'init'));
...
class FastWP_Actions {
	...
	static function  fastwp_setup(){
		...
		add_image_size			('fwp-hero-portfolio-thumb', 370, 241, true ); // Portfolio 370x241 image (cropped if larger)
		/* Not used */
		$args = array();
	...
	//	add_theme_support( 'custom-background', $args );
		...
		register_nav_menus( array(
			'primary' => esc_html__('Main navigation menu', 'heroes'),
		) );
	...
	static function enqueue_scripts(){
		...
		if(!is_admin()){
            ...
            wp_register_style( 'simple-line-icons', get_template_directory_uri() . '/assets/css/simple-line-icons.css', false, '1.0.0' );
	...
	static function init_visual_composer_params_blocks(){
		...
		if ((class_exists('WPBakeryVisualComposer') || class_exists('Vc_Manager')) && function_exists('vc_map')) {
			if(count($fwp_visual_composer_param_blocks) > 0 && is_array($fwp_visual_composer_param_blocks)){
				foreach( $fwp_visual_composer_param_blocks as $key => $vc_param_block ){
				    if( !in_array( $key, array( 'vc_row' ) ) ) {
				        continue;
                    }
    				foreach( $vc_param_block as $param ) {
	...
	static function attach_footer_info_page(){
        ...
        echo '<div class="topLine clearfix">
        <div class="container">
        <div class="row">';
	...
	static function set_custom_edit_post_columns($columns) {
    	$columns['item_id'] = esc_html__( 'Item ID', 'heroes' );
	   	return $columns;
	}
