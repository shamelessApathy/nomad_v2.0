	...
	**/
	static function register_scripts(){
		...
		foreach($fwp_load_scripts as $script){
			...
			if(substr_count($script,'http://')||substr_count($script,'https://')){
				$src = $script;
			} 
			
			wp_register_script( $handle, $src, $deps, '1.0.0', $in_footer );
			if(fwp_autoload_scripts == true || $handle == 'modernizr-custom' || $handle == 'scripts' || $handle == 'custom' ){
	...
	**/
	static function register_styles(){
		...
		foreach($fwp_load_styles as $style){
			...
			if(substr_count($style,'http://')){
				$src = $style;
			}
			wp_register_style( $handle, $src);
			if(fwp_autoload_styles == true || $handle == 'preloader'){
