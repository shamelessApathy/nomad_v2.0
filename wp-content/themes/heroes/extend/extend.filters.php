		...
		if(isset($meta['letter_parallax'])){
			...
			for($i=1;$i<=$max;$i++){
				if(isset($fwp__meta['text'.$i]) && !empty($fwp__meta['text'.$i])){
					...
					$_id		= rand(1000,9999);
					$addThisTpl = '<div class="parallaxIcon teamParallax%s" id="letter%s" data-stellar-ratio="%s" data-stellar-vertical-offset="%s"><h1><span>%s</span></h1></div>';
                    if( $i <= 2 )$letid = 1;
                    else $letid = 2;
					if(isset($meta['lplacement_t'.$i]) && $meta['lplacement_t'.$i] == '1'){
						$addThisA 	.= sprintf($addThisTpl, $letid, $_id, esc_attr($_ratio), esc_attr($_offset), esc_attr($_text));
					}else {
						$addThisB 	.= sprintf($addThisTpl, $letid, $_id, esc_attr($_ratio), esc_attr($_offset), esc_attr($_text));
					}
